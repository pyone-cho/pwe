...
echo "  2. Start services:   cd $APP_DIR/src/dev-deployment && docker compose -f docker-compose.dev.yml up -d --build"
echo "  3. Run migrations:   cd $APP_DIR/src/dev-deployment && docker compose -f docker-compose.dev.yml exec backend npx prisma migrate deploy"
